...
def events_to_timesurface(events):
    if len(events) == 0:
...
def extract_time_windows(events, t_start_us, t_end_us, window_us, stride_us):
    ...
    while current_t + window_us <= t_end_us:
        ...
        if len(window_events) < 100:
            ...
            current_t += stride_us
            continue
        samples.append(events_to_timesurface(window_events))
        current_t += stride_us
    return samples
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print(f"TIME SURFACE EXTRACTION ({args.window_ms}ms windows, 50% overlap)")
    print("=" * 60)
    ...
    print(f"Next step: python3 merge_timesurface.py --window_ms {args.window_ms}")