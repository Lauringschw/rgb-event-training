...
def merge(window_ms: int):
    ...
    print("=" * 60)
    print(f"MERGE TIME SURFACE: {window_ms}ms batches -> final dataset")
    print("=" * 60)
    print(f"Batches : {SLIDING_DIR}")
    print(f"Output  : {SLIDING_DIR_MERGED}\n")

    data_files  = sorted(SLIDING_DIR.glob("timesurface_data_batch_*.npy"),   key=get_batch_num)
    label_files = sorted(SLIDING_DIR.glob("timesurface_labels_batch_*.npy"), key=get_batch_num)
    recid_files = sorted(SLIDING_DIR.glob("timesurface_recids_batch_*.npy"), key=get_batch_num)
    ...
    sample_shape  = np.load(data_files[0]).shape[1:]   # (1, 360, 640)
    ...
    out_data   = SLIDING_DIR_MERGED / "timesurface_data.npy"
    out_labels = SLIDING_DIR_MERGED / "timesurface_labels.npy"
    out_recids = SLIDING_DIR_MERGED / "timesurface_recording_ids.npy"
    ...
    print("=" * 60)
    print(f"Output: {SLIDING_DIR_MERGED}")
    print(f"Next step: python3 train_timesurface.py --window_ms {window_ms}")
