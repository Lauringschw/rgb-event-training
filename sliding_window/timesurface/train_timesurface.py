...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print(f"TRAINING: {args.window_ms}ms Time Surface Model (ResNet-18)")
    print("=" * 60)
    print(f"Device     : {device}")
    print(f"Merged dir : {MERGED_DIR}\n")

    # == load ==================================================================
    data_path   = MERGED_DIR / "timesurface_data.npy"
    labels_path = MERGED_DIR / "timesurface_labels.npy"
    recids_path = MERGED_DIR / "timesurface_recording_ids.npy"

    for p in [data_path, labels_path, recids_path]:
        if not p.exists():
            raise FileNotFoundError(f"Missing: {p}\nRun merge_timesurface.py --window_ms {args.window_ms} first.")

    # Avoid loading full array into RAM
    ...
    # == model =================================================================
    model = ResNet18TimeSurface().to(device)
    ...
    best_val_acc      = 0.0
    epochs_no_improve = 0
    model_path        = MERGED_DIR / f'model_timesurface_{args.window_ms}ms_best.pth'
    ...
    print(f"Next step: python3 extract_test_samples.py --repr timesurface --window_ms {args.window_ms}")