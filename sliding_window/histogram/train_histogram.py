...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print(f"TRAINING: {args.window_ms}ms Histogram Model (ResNet-18)")
    print("=" * 60)
    print(f"Device     : {device}")
    print(f"Merged dir : {MERGED_DIR}\n")

    # == load ==================================================================
    data_path   = MERGED_DIR / "histogram_time_data.npy"
    labels_path = MERGED_DIR / "histogram_time_labels.npy"
    recids_path = MERGED_DIR / "histogram_time_recording_ids.npy"

    for p in [data_path, labels_path, recids_path]:
        if not p.exists():
            raise FileNotFoundError(f"Missing: {p}\nRun merge_histogram.py --window_ms {args.window_ms} first.")
    ...
    # == model =================================================================
    model = ResNet18Histogram(num_classes=3).to(device)
